...
#### MUI v9 and peer dependency alignment

This release raises supported peer versions to **MUI v9** and updates related tooling. Upgrade your app’s MUI, Emotion, React, and React Hook Form installs to match the peer ranges in `package.json` before adopting this version.

##### Peer dependency updates (high level)

- `@mui/material` and `@mui/system`: **^9.0.0**
- `@mui/x-date-pickers`: **^9.0.0**
- `react` / `react-dom`: **React 19** family (see `package.json` for the exact peer range)
- `react-hook-form`: **^7.74.0**

##### Library and docs

- Updated `FormFields`, `TextFieldController`, and `AutocompleteController` (and related typings) for MUI v9 compatibility
- Storybook stories adjusted for v9 APIs
- README and Chromatic Storybook link refreshed
